...
**利用可預測 ISN 的 TCP 欺騙：** 多個網際網路標準及相關提案討論了由於 _可預測_ 的初始序號導致的 TCP IP 欺騙問題。早在 1985 年，Morris 就觀察到 4.2BSD 在 TCP 中使用全域可預測的 ISN [29]——這是 RFC 793 中描述如何確定性地遞增全域 ISN 的結果。Morris 是第一個意識到並描述攻擊者如何濫用此弱點來創建 IP 欺騙的 TCP 連接的人，隨後 Bellovin 進行了分析 [1]。

為了克服可預測 ISN 的漏洞，Bellovin 首先在 RFC 1948 [2] 中，然後在 RFC 6528 [19] 中，提出按 4 元組（即源地址/端口、目標地址/端口）分隔序號空間。這樣，RFC 認為，IP 欺騙的 TCP 連接「只有在攻擊者已經有能力執行中間人攻擊時才可能實現」——這個評估在 2023 年由於網路速度顯著提高而不再成立。雖然 Cnot 早在 2012 年就已正確得出結論「主機不應基於 IP 地址建立信任關係」[18]，但 SPP 是違反此建議的一個完美例子。自那時起，對所提議的 TCP 連接進行了標準化，他們考慮了 IP 欺騙的 TCP 連接。

**透過暴力破解進行 TCP 欺騙：** 關於 TCP 欺騙的研究主要集中在如何建立一個 IP 欺騙的 TCP 連接。盲 TCP 欺騙由於在沒有高效用戶原語的情況下利用欺騙連接的成本很高，其效用有限。被稱為其的盲 TCP 欺騙攻擊簡單地利用了負載搭便車 [26]，並具有第 2 節中提到的限制。關於兩個負載傳輸原語的研究表明，攻擊者可以有效地濫用 IP 欺騙的 TCP 連接來啟動密集對話。

值得注意的先前工作描述了如何通過濫用 SYN Cookie 來優化握手欺騙。早期在任何給定波中編碼 Cookie 的提案迅速擴展到 32 位以增加所有 203 的搜索空間，Lell 的一篇部落格文章 [26] 實際展示了 SYN Cookie 縮小了 USV 欺騙的搜索空間，並且與我們的工作更為正交。與我們不同，他們的重點在於握手階段，而我們研究的是如何通過欺騙的連接傳輸負載的機制。

與基於暴力破解的 TCP 欺騙攻擊相比，Qam 和 Mao 在 [36] 中介紹了一種可以推斷伺服器端 ISN 的側信道，因此可以高效地建立和使用欺騙的 TCP 連接。然而，該攻擊對攻擊者能力有若干要求，這些要求不適用於我們論文中的反饋通道。首先，該攻擊需要一個在路徑上的序號檢查防火牆，該防火牆會丟棄具有視窗外序號的數據包。其次，該攻擊需要一個共享的響應式中間迴路與一個全域 IPID 控制器，以驗證探測數據包是否通過了防火牆。然而，眾所周知，全域 IPID 計數器是一個有噪聲的側信道 [9, 14]。此外，由於側信道威脅，大多數操作系統（以及因此，在路徑上的錯誤）放棄了全域 IPID 分配，轉而採用不可預測的 IPID 生成方案 [21, 39]。最後，攻擊者需要共享受害者客戶端和受害者伺服器之間路徑的某些部分。因此，攻擊者不能僅僅欺騙任意 IP 地址，而必須欺騙符合此要求的地址。

我們的論文遵循一個不那麼嚴格的威脅模型。這裡，攻擊者只需要具備欺騙能力，但因此必須暴力破解伺服器 ISN 以建立連接。一旦連接通過反饋通道成功建立，正確的工具就會得到確認，這些反饋通道即使在沒有中間盒或全域 IPID 計數器的情況下也能工作。

### 6.2 TCP 劫持/注入

其他一些工作針對現有 TCP 流的 TCP 注入攻擊，這與我們關於發起 IP 欺騙連接的工作是正交的。2007 年，RFC 4953 引入了針對盲連接終止攻擊的對策，該攻擊濫用 IP 欺騙的 RST 段來拆除 TCP 連接 [47]。Feng 等人展示了如何濫用分片來洩漏 TCP 連接的序號 [14, 15]。同樣地，Cao 等人 [9] 表明，如果早期具有全域 ACK 限制的 TCP/IP 實現獲得了現有連接的序號，則可能被濫用。Gilad 和 Herzberg [17] 以及 Mur [27] 濫用一個全域遞增的 IPID 作為反饋通道來了解 TCP 段是否落在窗口內——這可能重新評估了最後一方使其自身過時。Qam 等人 [36, 37] 利用協作的用戶空間惡意軟體來發起注入攻擊；這與我們的威脅模型不同。此外，Wi-Fi 實現中的漏洞允許攻擊者向現有 TCP 流中注入數據 [10]。

總體而言，TCP 劫持旨在劫持持久的 TCP 連接。然而，並非所有應用程式都會維持持久的 TCP 連接。例如，提議該研究的兩個用例，即 SMTP 和 PostgreSQL，就不是 TCP 劫持的理想目標。與 TCP 劫持攻擊研究中提出的各種洩漏序號和確認號的方法相比，我們研究的是向攻擊者發起的欺騙連接中注入負載。我們應用特定的反饋通道也可用於 TCP 劫持攻擊，以確認成功注入。

### 6.3 其他基於欺騙的攻擊

IP 欺騙在其他背景下已被廣泛研究。大量工作處理了 IP 欺騙的 SYN 泛洪攻擊 [12, 23, 46, 51]，而 SYN Cookie 可以防禦此類攻擊。IP 欺騙也可用於在 UDP 和 TCP 中發起反射放大攻擊 [5, 29, 38]。

## 7 結論

幾十年來我們已經知道，攻擊者可以通過暴力破解努力創建 IP 欺騙的 TCP 連接。但研究在很大程度上忽略了研究向欺騙的 TCP 連接注入負載的實際方面。我們的工作表明，存在可靠的用戶原語，攻擊者也可以通過欺騙的 TCP 連接發送用於 TCP 欺騙的數據，無論底層應用層協議為何。這再次強調，應用層協議不應僅依賴 IP 地址進行任何安全關鍵檢查，例如身份驗證或防火牆——即使使用了 TCP。可悲的是，應用程式仍然依賴 IP 地址，我們展示了 SMTP 和 PostgreSQL 這兩個用例。我們的工作表明，鑑於攻擊者具有繞過基於 IP 的訪問控制的實際影響，相互信任絕不能僅基於 IP 地址。我們希望本文及我們協調的披露能在未來提高對這種威脅的認識。
